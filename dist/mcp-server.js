...
function createMahaServer() {
    const server = new Server({ name: "Maha-OS-Agentic-Gateway", version: "1.0.2" }, { capabilities: { resources: {}, tools: {} } });
...
const lastBreakerFiredAt = new Map(); // nodeId -> epoch ms
function canFireCircuitBreaker(nodeId) {
    const now = Date.now();
...
app.post("/mcp", verifyAgentToken, async (req, res) => {
    try {
